...
spark = get_spark_session("BronzeToSilver-OrgType")

# 1. Read Bronze as a Stream
bronze_df = (spark.read
    .format("delta")
    #.option("inferSchema", "true")
    .load("s3a://lakehouse/bronze/orgtype"))

# 2. Clean the Data
# Drop rows where critical fields are null
cleaned_df = bronze_df.dropna(subset=["code", "description"])
...
spark.sql("""
    CREATE TABLE IF NOT EXISTS silver.orgtype
    USING delta
    LOCATION 's3a://lakehouse/silver/orgtype'
""")
...
#     .format("delta")
#     .outputMode("append")
#     .option("checkpointLocation", "s3a://lakehouse/checkpoints/silver/orgtype")
#     .trigger(availableNow=True) # 👈 THE MAGIC TRICK: Process new data and shut down
#     .start("s3a://lakehouse/silver/orgtype"))
...
query= (deduplicated_df.write 
    ...
    .mode("overwrite") 
    .saveAsTable("s3a://lakehouse/silver/orgtype")
)
