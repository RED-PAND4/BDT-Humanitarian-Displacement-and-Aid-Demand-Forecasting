...
spark.sql("""
    CREATE TABLE IF NOT EXISTS silver.sector
    USING delta
    LOCATION 's3a://lakehouse/silver/sector'
""")
...
#     .format("delta")
#     .outputMode("append")
#     .option("checkpointLocation", "s3a://lakehouse/checkpoints/silver/sector")
#     .trigger(availableNow=True) # 👈 THE MAGIC TRICK: Process new data and shut down
#     .start("s3a://lakehouse/silver/sector"))
...
query= (deduplicated_df.write 
    ...
    .mode("overwrite") 
    .saveAsTable("s3a://lakehouse/silver/sector")
)
