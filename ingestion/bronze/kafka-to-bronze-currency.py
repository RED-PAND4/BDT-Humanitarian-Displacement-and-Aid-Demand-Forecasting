...
if __name__ == "__main__":
    spark = get_spark_session("KafkaToBronze-Currency")
    ...
    # Maintain the exact variable names from the API
    my_fields_to_keep = {
        "code": "code",
        "name": "name"
    }
    ...
    spark.sql("""
        CREATE TABLE IF NOT EXISTS bronze.currency
        USING delta
        LOCATION 's3a://lakehouse/bronze/currency'
    """)
    ...
    CHECKPOINT_PATH = "s3a://lakehouse/checkpoints/bronze/currency"
    ...
    delta_query = (
        ...
        .start("s3a://lakehouse/bronze/currency")
    )
